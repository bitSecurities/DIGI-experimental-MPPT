...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
	...
	if (tx.IsCoinBase())
	{
		...
		if (tx.vin.size()==2)
		{
			string msg(tx.vin[1].scriptSig.begin(),tx.vin[1].scriptSig.end());

			if (!checkMppt(msg))
				return state.DoS(100, error("CheckTransaction() : mppt signature failed"),
						REJECT_INVALID, "bad-mppt-signature");
		}
	}
	else
	{
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
	...
	// DIGI: Reject block.nVersion=1 blocks (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
	...
	// DIGI: (mainnet >= 710000, testnet >= 400000, regtest uses supermajority)
